...
export default function UnratedMatches() {
  ...
  const [discountEnd] = React.useState(() => {
    const end = new Date();
    end.setHours(end.getHours() + 8, end.getMinutes() + 0, end.getSeconds() + 41);
    return end.getTime();
  });
  ...
  const basePerGame =
    queueType === "Solo"
      ? BASE_PRICE_PER_GAME
      : BASE_PRICE_PER_GAME * 1.3 * (1 + (duoBoosterCount - 1) * 0.25);
  const platformMultiplier = platform === "PC" ? 1 : 1.2;
  const boosterFee = specificBooster ? 7.5 : 0;

  let subtotal = numberOfGames * basePerGame * platformMultiplier + boosterFee;
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({ total, numberOfGames, server, platform }),
      });
  ...
  const faqCopy = {
    label: "FAQs",
    items: [
      { q: "What is Unrated Matches Boost?", a: "Our Unrated Matches Boost service has a skilled booster play the selected number of unrated games on your account. There's no win requirement — every game counts toward your order." },
      { q: "Does win rate matter for unrated?", a: "No. Unrated matches don't affect your competitive rank. Our boosters still play to win, but there's no penalty for losses in this mode." },
      ...
      { q: "How long does each unrated game take?", a: "Roughly 30–45 minutes per match. Express delivery reduces queue and session time significantly." },
      { q: "Can I request specific operators?", a: "Yes! Enable the 'Specific Operators' add-on and submit your preferences after checkout." },
    ],
  };

  const infoCopy = {
    sections: [
      { heading: "Play Unrated Without the Grind", paragraphs: ["Unrated matches in Rainbow Six Siege are the perfect way to learn new operators, complete challenges, or just enjoy the game without ranked pressure. Our boosters handle the grind while you focus on what matters."] },
      { heading: "What Does Unrated Boost Include?", paragraphs: ["You select the number of games you want completed. One of our verified boosters will play on your account (Solo) or alongside you (Duo) in unrated mode until all games are delivered."] },
      { heading: "No Win Rate Stress", paragraphs: ["Since unrated doesn't affect your competitive MMR, this service is completely consequence-free. Perfect for completing seasonal challenges or testing loadouts without risk."] },
      { heading: "All Platforms Supported", paragraphs: ["Available on PC, Xbox, and PlayStation. Console orders carry a 20% surcharge due to the smaller pool of available console boosters."] },
    ],
  };
      ...
      <div className="relative mx-auto max-w-[1550px] px-6 py-8 pt-44">
        <header className="mb-6">
          <h1 className="text-4xl font-extrabold tracking-tight sm:text-5xl">
            Rainbow Six Siege Unrated Matches Boost
          </h1>
          <p className="mt-3 text-sm leading-6 text-zinc-400 sm:text-base">
            Affordability, practicality, and peace of mind are the pillars of our vision for the Rainbow Six Siege Boost in Unrated Matches. Across all regions &amp; platforms.
          </p>
              ...
              <div className="relative rounded-3xl border border-white/10 bg-black/40 p-6 overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="mb-6 flex flex-col items-start">
                    <Image src="/rainbow-six-siege-unrated-matches-boost.webp" alt="Unrated" width={56} height={56} unoptimized className="mb-0 h-14 w-14 object-contain drop-shadow-[0_0_12px_rgba(34,211,238,0.3)]" />
                    <h2 className="text-2xl font-extrabold text-white tracking-tight">Number of Games</h2>
                    <p className="text-cyan-400 text-base font-medium mt-1">Select the Number of Games</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => setNumberOfGames(Math.max(1, numberOfGames - 1))}
                    ...
                    <div className="flex-1 flex items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] h-12">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!isNaN(v)) setNumberOfGames(Math.min(50, Math.max(1, v)));
                        }}
                    ...
                    <button
                      onClick={() => setNumberOfGames(Math.min(50, numberOfGames + 1))}
                      className="flex h-12 w-16 items-center justify-center rounded-xl bg-white/[0.06] text-white font-bold text-xl hover:bg-white/[0.1] active:scale-95 transition cursor-pointer"
                    >+</button>
                  </div>
                  <p className="mt-3 text-xs text-zinc-500">Max 50 games per order. For bulk orders, please contact support.</p>
                </div>
              </div>
            </section>
          ...
          <aside className="h-fit rounded-3xl border border-white/10 bg-black/50 p-6 backdrop-blur-sm xl:sticky xl:top-6 shadow-lg shadow-cyan-500/10">
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 text-sm text-zinc-400">
                <span>~ {Math.max(1, Math.round(numberOfGames * 0.5))}h</span>
