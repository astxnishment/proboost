...
import { Clock3, Mail, ShieldCheck, UserRound, Zap } from "lucide-react";
...
export default function AuthScreen({ mode }: AuthScreenProps) {
  ...
  return (
    <main className="auth-shell flex min-h-[calc(100vh-var(--header-height))] items-center py-10 sm:py-14">
      ...
      <div className="content-container grid w-full gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(380px,0.78fr)] lg:items-stretch">
        <section className="theme-preserve-media relative hidden min-h-[660px] overflow-hidden rounded-[var(--radius-featured)] border border-[var(--line)] bg-[#09090b] text-white lg:block">
          <Image
            src="/homepage/r6-homepage.webp"
            alt="Rainbow Six Siege specialist"
            fill
            loading="eager"
            fetchPriority="high"
          ...
          <div className="absolute inset-x-0 bottom-0 p-8 xl:p-10">
            <p className="text-sm font-semibold text-white/75">Your ProBoost account</p>
            <h2 className="mt-3 max-w-[11ch] text-3xl font-semibold leading-tight text-white xl:text-4xl">
              Keep every order in one place.
            </h2>
            <div className="mt-7 grid gap-4">
              {[
                { icon: Zap, title: t.fastTitle, description: t.fastDesc },
        ...
        <section className="surface flex items-center p-6 sm:p-8 lg:p-10">
          <div className="mx-auto w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="mt-7 grid gap-4">
              {mode === "signup" && (
                <div>
                  <label htmlFor="auth-username" className="mb-2 block text-sm font-semibold">
                    Username
                  </label>
                  <div className="relative">
                    ...
                    />
                    <UserRound aria-hidden className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted)]" />
                  </div>
                </div>
              )}

              <div>
                <label htmlFor="auth-email" className="mb-2 block text-sm font-semibold">
                  {t.emailLabel}
                </label>
                <div className="relative">
                  <input
                    ...
                    className="field-control pr-11"
                  />
                  <Mail aria-hidden className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted)]" />
                </div>
              </div>

              <div>
                <label htmlFor="auth-password" className="mb-2 block text-sm font-semibold">
                  {t.passwordLabel}
                </label>
                <input
                  id="auth-password"
              ...
              {mode === "signup" && (
                <div>
                  <label htmlFor="auth-password-confirm" className="mb-2 block text-sm font-semibold">
                    {t.confirmLabel}
                  </label>
                  <input
              ...
              {error && (
                <p id="auth-error" role="alert" className="theme-error rounded-lg px-4 py-2.5 text-sm">
                  {error}
                </p>
              )}
              ...
              <button
                type="submit"
                disabled={loading}
                className="button-base button-primary mt-1 w-full"
              >
              ...
              {mode === "login" && (
                <p className="text-center text-xs text-[var(--muted)]">
                  Trouble logging in?{" "}
                  <a href="mailto:support@proboost.gg" className="font-semibold text-[var(--accent)]">
                    Get help
                  </a>
                </p>
              )}
            ...
            <div className="my-6 flex items-center gap-3 text-xs text-[var(--muted-soft)]">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className="button-base button-secondary px-3"
              >
                <svg className="h-5 w-5" viewBox="0 0 127.14 96.36" fill="currentColor" aria-hidden>
            ...
            <p className="mt-6 text-center text-sm text-[var(--muted)]">
              {content.altLabel}{" "}
              <Link href={altHref} className="font-semibold text-[var(--accent)]">
                {content.altText}
              </Link>
            </p>
          </div>
